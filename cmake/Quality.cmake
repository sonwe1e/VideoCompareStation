...
function(_dvs_require_llvm_tool_version executable toolName)
    ...
    if(NOT "${dvs_tool_version}" STREQUAL "${dvs_required_llvm_tool_version}")
        message(FATAL_ERROR
            "CompareStation requires ${toolName} ${dvs_required_llvm_tool_version}, but '${executable}' "
...
function(dvs_add_quality_targets)
    ...
    if(NOT "$ENV{VCINSTALLDIR}" STREQUAL "")
        file(TO_CMAKE_PATH "$ENV{VCINSTALLDIR}" dvs_vc_install_dir)
        cmake_path(APPEND dvs_vc_install_dir "Tools/Llvm/x64/bin"
                   OUTPUT_VARIABLE dvs_vc_llvm_bin_dir)
        list(APPEND dvs_quality_tool_hints "${dvs_vc_llvm_bin_dir}")
    endif()
    if(NOT "$ENV{VCToolsInstallDir}" STREQUAL "")
        file(TO_CMAKE_PATH "$ENV{VCToolsInstallDir}" dvs_vc_tools_install_dir)
        # vcvarsall includes a trailing separator; strip it before walking up from the version.
        string(REGEX REPLACE "/+$" "" dvs_vc_tools_install_dir "${dvs_vc_tools_install_dir}")
    ...
    if(MSVC)
        set(dvs_msvc_dependency_check
            "${CMAKE_COMMAND}"
            "-DDVS_BINARY_DIR=${CMAKE_BINARY_DIR}"
            "-DDVS_SOURCE_DIR=${PROJECT_SOURCE_DIR}"
            "-DDVS_NINJA_EXECUTABLE=${CMAKE_MAKE_PROGRAM}"
            -P "${PROJECT_SOURCE_DIR}/cmake/CheckMsvcDependencies.cmake"
        )
        add_custom_target(
            dvs_check_msvc_dependencies
            COMMAND ${dvs_msvc_dependency_check}
            DEPENDS dvs_domain
            VERBATIM
        )
        add_dependencies(format-check dvs_check_msvc_dependencies)
        add_dependencies(lint dvs_check_msvc_dependencies)
        if(BUILD_TESTING)
            add_test(NAME quality.msvc_dependencies COMMAND ${dvs_msvc_dependency_check})
            set_tests_properties(
                quality.msvc_dependencies
                PROPERTIES LABELS "quality;unit;app" TIMEOUT 15
            )
            add_test(
                NAME quality.msvc_dependency_contracts
                COMMAND "${CMAKE_COMMAND}"
                    "-DDVS_TEST_ROOT=${CMAKE_BINARY_DIR}/Testing/dependency-contracts"
                    "-DDVS_NINJA_EXECUTABLE=${CMAKE_MAKE_PROGRAM}"
                    -P "${PROJECT_SOURCE_DIR}/cmake/CheckMsvcDependenciesTests.cmake"
            )
            set_tests_properties(
                quality.msvc_dependency_contracts
                PROPERTIES LABELS "quality;unit;app" TIMEOUT 30
            )
        endif()
    endif()
    ...
    if(DVS_BUILD_DESKTOP)
        add_dependencies(lint CompareStation CompareStationCli)
    else()
